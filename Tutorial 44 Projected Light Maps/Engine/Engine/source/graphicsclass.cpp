...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	result = m_GroundModel->Initialize(m_D3D->GetDevice(), m_D3D->GetDeviceContext(), "../Engine/data/floor.txt", "../Engine/data/stone.tga");
	if(!result)
	{
	...
	result = m_CubeModel->Initialize(m_D3D->GetDevice(), m_D3D->GetDeviceContext(), "../Engine/data/cube.txt", "../Engine/data/seafloor.tga");
	if(!result)
	{
	...
	result = m_ProjectionTexture->Initialize(m_D3D->GetDevice(), m_D3D->GetDeviceContext(), "../Engine/data/grate.tga");
	if(!result)
	{
	...
	m_ViewPoint->SetPosition(2.0f, 5.0f, -8.0f);
	m_ViewPoint->SetLookAt(0.0f, 0.0f, 0.0f);
	m_ViewPoint->SetProjectionParameters((3.141592654f / 2.0f), 1.0f, 0.1f, 1000.0f);
